...
模型现在分别生成查询词、检索类型、时间范围、来源域名和来源偏好。例如“最近的 AI 新闻”可以用 `AI product launches` 配合 `intent: news`、`timeRange: week`，先发现事件，再围绕已发现的产品查找原始公告。提示词会要求覆盖模型、产品和行业等不同角度，避免把日期、arXiv 和 `(official blog OR paper)` 堆进一条搜索词；不会按固定词数截断查询，也没有预设厂商白名单。“最近新闻”没有明确时间时，规划默认采用近一周。

| 搜索服务 | 已接通的原生参数 | 限制 |
| --- | --- | --- |
| Tavily | 类型、相对时间或起止日期、域名、语言偏好 | 语言偏好不保证结果全部采用该语言；单次任务类型优先于全局设置，旧计划仍使用全局设置兜底。 |
| Firecrawl | 网页/新闻入口、`tbs` 时间条件、域名 | 当前安装的搜索 SDK 不支持语言过滤；新闻没有抓取正文时仍保留摘要。 |
| Google PSE | `dateRestrict` 相对时间、单个域名、语言 | 不假装支持新闻专用入口、任意发布日期区间或多域名单站限制。 |
| CRW | 原有基本检索与抓取 | 不假定它支持所有 Firecrawl 扩展参数。 |

节点详情展示检索类型、时间范围、是否改写重查、服务未应用的过滤条件，以及来源提供的日期。这些信息会保存到历史记录，导入后重试仍可沿用原计划。来源日期不等同于事件发生日期；供应商返回的相关性分数会保留，但不跨供应商比较。

相关性判断复用原有的证据提取调用，同时核对主题和时间。结论必须来自检索返回的 URL，且带有能与原文匹配的摘录。没有找到合格证据时，最多使用模型提出的简化查询重查一次，所有过滤条件保持不变；深入研究也继承父节点的时间范围。网络错误、鉴权错误不会触发搜索词改写。每个节点最多两次应用层搜索尝试，不含供应商 SDK 自身的传输重试。因此，定向补查仍是两个方向、一轮深度，每个方向最多重查一次。服务不支持的过滤条件会明确提示，并交由来源内容核查，无法保证完整召回。

`pnpm test` 覆盖参数映射、实际请求、语言传递、空结果和无关结果、一次重查上限、日期继承、无效证据、取消和历史兼容。真实搜索质量仍取决于模型和搜索服务。对比新旧效果时，可固定模型、供应商、广度和深度，分别测试“最近的 AI 新闻”“2026 年 9 月 1–7 日的 AI 产品发布”、一个具名产品的官方公告，以及一个不涉及新闻的技术比较；记录相关且不重复的事件数、日期与原始来源证据、耗时和调用次数。模拟测试不能代替真实相关性评估。
