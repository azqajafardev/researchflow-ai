...
**researchflow-ai** turns a research question into a cited report: it plans searches, browses the web, verifies findings against real source pages, and lets you inspect and follow up on every citation — all from your browser, or self-hosted with your own API keys.

> **Project naming:** This README uses **researchflow-ai** as the display name. Existing repository links, Docker images, live demos, sponsors, and star history below belong to the original Deep Research Web UI project. The deployment commands retain their original identifiers so they remain usable.
...
- 🤖 **Supports more models**: Uses plain prompts instead of newer, less widely supported features like Structured Outputs. This ensures compatibility with more providers.
- 🐳 **Docker support**: Deploy in your environment with a one-line command
- 🔧 **Server Mode**: Deploy with environment variables; users do not need to configure API keys
...
- 📖 **On-demand full-page reading**: Fetch full source pages to extract verbatim evidence when search snippets aren't enough, sharing a token/request budget across the whole research run
...
- Web Search: Tavily, [Firecrawl](https://firecrawl.dev) (cloud / self-hosted), fastCRW (cloud / self-hosted), Google PSE
...
1. After research finishes, click a `[1]`-style citation in the report, or click **Inspect evidence and follow up**.
2. In the evidence panel, review each finding's source, retrieval time, and excerpt. A new excerpt is only saved once matched against retrieved content; search summaries and page text are labeled separately. A matched excerpt does not by itself prove the finding — read it in context.
...
Original project live demo: <a href="https://deep-research.ataw.top" target="_blank">https://deep-research.ataw.top</a>
...
Deploy with environment variables when you can run the Nuxt server:
...
One-click deploy of the original repository with [EdgeOne Pages](https://edgeone.ai/products/pages):
...
Use the original pre-built Docker image:

```bash
docker run -p 3000:3000 --name deep-research-web -d anotia/deep-research-web:latest
```

Build the original repository yourself:
...
- fastCRW (`crw`) is a Firecrawl-compatible web scraper (single binary; self-host or cloud). It defaults to `https://fastcrw.com/api` and reads the key from `NUXT_WEB_SEARCH_API_KEY` (document as `CRW_API_KEY`); self-hosted deployments can set `NUXT_WEB_SEARCH_API_BASE`.
- Ollama uses `http://localhost:11434/v1` as the default API base. Inside Docker, `localhost` refers to the container, so set `NUXT_AI_API_BASE` to a reachable host or Docker network address if Ollama runs outside the container.
...
See the [Nuxt deployment documentation](https://nuxt.com/docs/getting-started/deployment) for details.
